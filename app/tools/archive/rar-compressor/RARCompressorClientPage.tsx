...
export function RARCompressorClientPage() {
  ...
  const handleFilesSelected = useCallback(
    (selectedFiles: File[]) => {
      ...
      const rarFiles = selectedFiles.filter(
        (file) =>
      ...
      if (rarFiles.length < selectedFiles.length) {
        toast({
          title: "Unsupported file type",
          description: "Only RAR files are supported for this tool.",
          variant: "destructive",
        })
      }
  ...
  const compressFiles = async () => {
    ...
    try {
      for (const file of files) {
        // Note: Browser-based RAR decompression is limited
        // In a real implementation, we would use a server-side solution
        // For this demo, we'll convert RAR to ZIP with high compression

        // Create a new ZIP with high compression
        const zip = new JSZip()

        // Add the RAR file as-is to the ZIP (we can't decompress it in the browser)
        ...
        // Change the extension from .rar to .zip
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          <CompressedFilesList files={compressedFiles} onDelete={handleDeleteFile} toolName="RAR Compressor" />
        </div>
      </div>
    </div>
  )
