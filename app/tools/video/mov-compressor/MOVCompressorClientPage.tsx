...
const MOVCompressorClient = () => {
  ...
  const handleFilesSelected = useCallback(
    (selectedFiles: File[]) => {
      // Filter only MOV files
      const movFiles = selectedFiles.filter(
        (file) => file.type === "video/quicktime" || file.name.toLowerCase().endsWith(".mov"),
      )

      if (movFiles.length < selectedFiles.length) {
        toast({
          title: "Unsupported file type",
          description: "Only MOV files are supported for this tool.",
          variant: "destructive",
        })
      }
  ...
  const compressFiles = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i]
        const safeFileName = `input_${i}_${Date.now()}.mov`
    ...
    <div className="space-y-8 flex flex-col items-center">
      <div className="w-full max-w-4xl">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div>
            <CompressedFilesList files={compressedFiles} onDelete={handleDeleteFile} toolName="MOV Compressor" />
          </div>
        </div>
      </div>
    </div>
...
export const MOVCompressorClientPage = dynamic(() => Promise.resolve(MOVCompressorClient), {
  ssr: false,
})
