...
const AVICompressorClient = () => {
  ...
  const handleFilesSelected = useCallback(
    (selectedFiles: File[]) => {
      ...
      const aviFiles = selectedFiles.filter(
        (file) => file.type === "video/x-msvideo" || file.name.toLowerCase().endsWith(".avi"),
      )

      if (aviFiles.length < selectedFiles.length) {
        toast({
          title: "Unsupported file type",
          description: "Only AVI files are supported for this tool.",
          variant: "destructive",
        })
      }
  ...
  const compressFiles = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i]
        const safeFileName = `input_${i}_${Date.now()}.avi`
    ...
    <div className="space-y-8 flex flex-col items-center">
      <div className="w-full max-w-4xl">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div>
            <CompressedFilesList files={compressedFiles} onDelete={handleDeleteFile} toolName="AVI Compressor" />
          </div>
        </div>
      </div>
    </div>
...
export const AVICompressorClientPage = dynamic(() => Promise.resolve(AVICompressorClient), {
  ssr: false,
})
