...
export function ImageCompressorClientPage() {
    ...
    (selectedFiles: File[]) => {
      ...
      if (imageFiles.length < selectedFiles.length) {
        toast({
          ...
          description: "Only image files are supported for this tool.",
          variant: "destructive",
        })
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          <CompressedFilesList files={compressedFiles} onDelete={handleDeleteFile} toolName="Image Compressor" />
        </div>
      </div>
    </div>
  )
